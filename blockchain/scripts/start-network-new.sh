...
generate_crypto_materials() {
    ...
    # Run the generate-crypto-materials.sh script
    "${BLOCKCHAIN_DIR}/scripts/generate-crypto-materials.sh"
...
generate_genesis_block() {
    ...
    # Run the generate-genesis-block-new.sh script
    "${BLOCKCHAIN_DIR}/scripts/generate-genesis-block-new.sh"
...
generate_channel_artifacts() {
    ...
    # Run the generate-channel-artifacts-new.sh script
    "${BLOCKCHAIN_DIR}/scripts/generate-channel-artifacts-new.sh"
...
create_channels() {
    ...
    docker exec cli bash -c "peer channel create -o orderer.election-commission.gov.in:7050 -c votingchannel -f /opt/gopath/src/github.com/hyperledger/fabric/peer/channel-artifacts/voting-channel.tx"
    
    # Join peer0.state.gov.in to the channel
    docker exec cli bash -c "peer channel join -b votingchannel.block"
    
    # Join peer1.state.gov.in to the channel
    docker exec -e CORE_PEER_ADDRESS=peer1.state.gov.in:7051 cli bash -c "peer channel join -b votingchannel.block"
    
    # Join peer0.district.gov.in to the channel
    docker exec -e CORE_PEER_LOCALMSPID=DistrictElectionOfficeMSP -e CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto-config/peerOrganizations/district.gov.in/users/Admin@district.gov.in/msp -e CORE_PEER_ADDRESS=peer0.district.gov.in:9051 cli bash -c "peer channel join -b votingchannel.block"
    
    # Join peer1.district.gov.in to the channel
    docker exec -e CORE_PEER_LOCALMSPID=DistrictElectionOfficeMSP -e CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto-config/peerOrganizations/district.gov.in/users/Admin@district.gov.in/msp -e CORE_PEER_ADDRESS=peer1.district.gov.in:9051 cli bash -c "peer channel join -b votingchannel.block"
    
    # Update anchor peers for StateElectionOfficeMSP
    docker exec cli bash -c "peer channel update -o orderer.election-commission.gov.in:7050 -c votingchannel -f /opt/gopath/src/github.com/hyperledger/fabric/peer/channel-artifacts/StateElectionOfficeMSPanchors.tx"
    
    # Update anchor peers for DistrictElectionOfficeMSP
    docker exec -e CORE_PEER_LOCALMSPID=DistrictElectionOfficeMSP -e CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto-config/peerOrganizations/district.gov.in/users/Admin@district.gov.in/msp -e CORE_PEER_ADDRESS=peer0.district.gov.in:9051 cli bash -c "peer channel update -o orderer.election-commission.gov.in:7050 -c votingchannel -f /opt/gopath/src/github.com/hyperledger/fabric/peer/channel-artifacts/DistrictElectionOfficeMSPanchors.tx"
